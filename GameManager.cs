...
public class GameManager : NetworkBehaviour
{
    ...
    {
        Debug.Log($"GM Spawn Server={IsServer}");
        Debug.Log(
            $"Board Spawned Owner={OwnerClientId} Local={NetworkManager.Singleton.LocalClientId} Server={IsServer}"
        );
        // Només el Host/Servidor té permís per crear objectes a la xarxa
        if (IsServer)
        {
            SpawnBoardForPlayer(0);
            SpawnBoardForPlayer(1);
            StartGameServerRpc();
        }
    }

    // --- NEW: Registration Logic ---
    public void RegisterBoard(ulong ownerId, BoardManager board)
    {
        // In Unity Netcode, the Host always has the ServerClientId (usually 0)
        if (ownerId == NetworkManager.ServerClientId)
        {
            player1.clientId = ownerId;
            player1Board = board;
            player1Board.gameObject.name = $"Board_P1_Host_{ownerId}";
        }
        else
        {
            player2.clientId = ownerId;
            player2Board = board;
            player2Board.gameObject.name = $"Board_P2_Client_{ownerId}";
        }
    }

    // --- UPGRADED: Strict Online Click Validation ---
    public bool IsCellClickValid(BoardManager boardClicked)
    {
        if (CurrentState == GameState.Initiative) return false;

        // Security 1: Is it my turn? (Check if the active player matches the local computer)
        ...
        // Security 2: Are you clicking YOUR OWN board?
        if (boardClicked.OwnerClientId != NetworkManager.Singleton.LocalClientId)
        {
    ...
    public void SpawnBoardForPlayer(ulong clientId)
    {
        Debug.Log($"Spawn board for {clientId}");
        // 1. Instantiate the prefab (it will be created on the server)

        GameObject boardInstance = Instantiate(networkBoardPrefab);

        // 2. Set the parent IMMEDIATELY after instantiation, but BEFORE spawning
        Canvas mainCanvas = FindAnyObjectByType<Canvas>();
        if (mainCanvas != null)
        {
            // Use SetParent with worldPositionStays = false to keep local scale/orientation
            boardInstance.transform.SetParent(mainCanvas.transform, false);
        }

        // 3. Now that the parent is set, spawn it on the network
        NetworkObject networkObject = boardInstance.GetComponent<NetworkObject>();
        networkObject.SpawnWithOwnership(clientId);
        Debug.Log($"Spawned board NetworkObjectId={networkObject.NetworkObjectId}");

        Debug.Log($"[Network] Board spawned and parented for client {clientId}");
    }
